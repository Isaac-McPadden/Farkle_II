...
def test_simulation_completion_republishes_only_outputs_rewritten_during_resume(
    v3_cfg: AppConfig,
) -> None:
    cell = v3_cfg.n_dir(2)
    cell.mkdir(parents=True)
    ...
    workload = cell / "simulation_workload_plan.json"
    workload.write_text('{"required_shuffles": 1}\n', encoding="utf-8")
    checkpoint = cell / "2p_checkpoint.pkl"
    checkpoint.write_bytes(pickle.dumps({"complete": True}))
    outputs = [checkpoint, workload, strategy_manifest]
    ...
    first_sidecar = sidecar_path(checkpoint).read_bytes()

    # These are the two mutable outputs regenerated by a no-force resume.
    workload.write_text('{"required_shuffles": 2}\n', encoding="utf-8")
    checkpoint.write_bytes(pickle.dumps({"complete": True, "resumed": True}))
    ...
    write_simulation_done(
        ...
        n_strategies=1,
        outputs=outputs,
        rewritten_outputs=[checkpoint, workload],
    )
    assert sidecar_path(checkpoint).read_bytes() != first_sidecar
    for artifact in outputs:
        validate_authenticated_artifact_unbound(artifact, validate_provenance=False)
