...
def _normalize_meta_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["strategy_id"]
    if "players" in normalized:
        sort_cols = ["players", "strategy_id"]
    normalized.sort_values(sort_cols, inplace=True, kind="mergesort")
...
def run(cfg: AppConfig, *, force: bool = False, use_random_if_I2_gt: float | None = None) -> None:
    ...
    if pooled_frames:
        ...
        if force or not _parquet_matches(meta_long_path, meta_long):
            ...
            write_parquet_atomic(table, meta_long_path)
        else:
            LOGGER.info(
                "Meta long parquet already up-to-date",
                extra={"stage": "meta", "path": str(meta_long_path)},
            )
