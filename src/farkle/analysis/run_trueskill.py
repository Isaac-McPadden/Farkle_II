...
``ratings_<N>.parquet`` and ``ratings_pooled.parquet``
    ...
    ``07_trueskill/<Np>/`` and ``07_trueskill/pooled/`` respectively.
``tiers.json``
...
def run_trueskill(
    ...
    root: Path | None, optional
        ...
        ``<dataroot>/analysis/07_trueskill`` when ``None``.
    ...
    root = Path(root) if root is not None else base / "analysis" / "07_trueskill"
