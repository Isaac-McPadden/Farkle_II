...
The outputs are written to ``12_variance/pooled/variance.parquet`` and a compact
summary aggregated by ``n_players`` is written to
``12_variance/pooled/variance_summary.parquet``. The module also derives
seed-level variance components for win rate, total score, and game length and
writes them to ``12_variance/pooled/variance_components.parquet``. All outputs
