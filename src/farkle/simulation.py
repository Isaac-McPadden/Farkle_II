                    ...
                    for sf in smart_five_opts:  # 2 options
                        for so in smart_one_opts:  # 1.5 options
                            if not sf and so:  # Can't be smart one without being smart five
                                # You technically could but it makes no strategic
                                # sense in real games
                                continue
                            for cs in consider_score_opts:  # 2 options
                                # 2.5 options: quarter of cs&cd gets an
                                # extra option when require_both is True
                                for cd in consider_dice_opts:
                                    ...
                                    for rb in rb_values:
                                        ...
                                        # 1.6 options: increases cs, cd, rb truth table
                                        # from 5 to 8 with cs, cd, rb, ps (8/5=1.6)
                                        for ps in ps_values:
...
def experiment_size(
    *,
    ...
    # prefer score adds an extra option for each cs,cd TT (2 of them) or FF
    # combination
    pair_count += 3
