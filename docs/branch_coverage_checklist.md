...
- [x] `farkle.orchestration.run_contexts` - missing combine stage, preserved frequentist seeds, and interseed layout fallbacks (`None`/unknown layout) - `test_interseed_context_raises_when_combine_missing`, `test_interseed_context_preserves_existing_frequentist_seeds`, `test_run_context_config_interseed_folder_mapping_missing_key`.
- [x] `farkle.utils.mdd_helpers` - zero-division fallback without Jeffreys prior and missing-games default branch - `test_ensure_winrate_without_jeffreys_fills_nan_to_zero`, `test_prepare_cell_means_defaults_games_when_column_missing`.
