...
| `rng_diagnostics` | `analysis.rng_diagnostics.run()` publishes authenticated `diagnostics/rng_diagnostics.parquet`, `rng_diagnostics_summary.json`, and compact selection artifacts after four resumable partition phases. | Validates the authenticated `combine` concat source. Count routing, exact eligibility, selected-observation routing, and bounded local ordering use projected byte-bounded Arrow batches. Output is not consumed statistically, but its completion state participates in final root health. | Hash-bound unit stamps and ordered manifests precede `rng_diagnostics.done.json`; cache key 6; diagnostic method version 4, RNG 2, explicit strategy-seat and matchup-game semantics, descriptive zero-centered bands, deterministic semantic-priority cap, explicit estimability, and blocked-by-cap completion. Tests cover equivalence, worker/partition invariance, deterministic cap selection, corruption, interruption/resume, and sparse high cardinality. |
...
| Retired RNG diagnostic eager/SQLite helpers | Removed in method v4. The canonical implementation uses compact integer route records and exact external partition reducers only. |
