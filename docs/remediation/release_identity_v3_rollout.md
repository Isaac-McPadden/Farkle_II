...
| RNG diagnostics | Diagnostic output publishes with RNG-diagnostic method 2 and an authenticated concat source. | It is terminal diagnostic evidence; root lifecycle and final health require its v3 completion when enabled. |
...
| `rng_diagnostics` | `analysis.rng_diagnostics.run()` publishes `diagnostics/rng_diagnostics.parquet`. | Reads `combine` concat rows by path and required columns; no input-sidecar validation. Output is not consumed statistically, but its completion state participates in final root health. | `rng_diagnostics.done.json`; cache key 4; diagnostic method version 2 (`_DIAGNOSTIC_METHOD_VERSION`), RNG 2, tournament-player namespace, descriptive zero-centered band. **Still-v2 output; partially migrated completion.** Tests: `test_rng_diagnostics_branches.py::test_one_frame_semantic_sequence_matches_hand_oracle` and `test_fragmented_batches_and_seats_match_one_frame_oracle`; lifecycle health is covered by orchestration tests. |
