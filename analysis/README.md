...
- `--rng-diagnostics` – run `farkle.analysis.rng_diagnostics` after `combine` when interseed analytics are enabled, writing `rng_diagnostics.parquet` with a `rng_diagnostics.done.json` freshness stamp.
...
- `--rng-lags <int>` (repeatable) – provide one or more lag values for RNG autocorrelation diagnostics when running interseed analysis (defaults to `1`).
